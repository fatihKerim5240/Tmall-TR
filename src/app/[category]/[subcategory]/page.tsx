...
export async function generateStaticParams() {
  ...
  for (const cat of NAV_CATEGORIES) {
    for (const sub of cat.subCategories ?? []) {
      // sub.href is like "/kadin/ust" → split gives ["", "kadin", "ust"]
...
export default async function SubCategoryPage({ params }: Props) {
  ...
  // Find the matching subcategory by its href slug
  const subData = catData.subCategories?.find((s) => {
    const slug = s.href.split("/").pop();
    return slug === subcategory;
  });

  // Products are filtered at category level (sub-category tags not in data yet)
